...
from .views import BookUpdateCall,BookPostCall,Booksviewsets,SubCategoryViewSet,CategoryViewSet,book_GetItemByfield_InputView,Category_GetItemByfield_InputView,GetSubCategoryById_InputView
# from .views import Registerview,LoginApiView,VerifyOtpView,ResendOtp,ForgotOtp,ResetPassword

router = DefaultRouter()
router.register('category', CategoryViewSet, basename='category')
router.register('book', Booksviewsets, basename='book')
router.register('sub_category', SubCategoryViewSet, basename='sub_category')


urlpatterns = [
    path('', include(router.urls)),
    path('BookUpdateCall/', BookUpdateCall.as_view(), name='BookUpdateCall'),
